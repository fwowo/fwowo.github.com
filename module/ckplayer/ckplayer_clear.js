...
function ckstyle() {
    var ck = new Object();
    ck.cpath = ckPath + 'ckplayer/assets/';
	/*播放器文件的路径*/
	ck.mylogo=null;
	...
	*/
	...
	/*以下是播放器自带的二个插件*/
	ck.control_rel='null,' + ckPath + 'ckplayer/related.xml,0';
	...
	*/
	...
	ck.cpt_share = ckPath + 'ckplayer/share.xml';
	/*分享插件调用的配置文件地址*/
	//调用插件开始
    ck.cpt_list = ckcpt();
    return ck;
...
var ckPlayer = function(divId, width, height, mUrl, mPic, autoPlay) {
	...
	var attributes = {id:'ckplayer_' + divId,name:'ckplayer_' + divId};
	//下面一行是调用播放器了，括号里的参数含义：（播放器文件，要显示在的div容器，宽，高，需要flash的版本，当用户没有该版本的提示，加载初始化参数，加载设置参数如背景，加载attributes参数，主要用来设置播放器的id）
	swfobject.embedSWF(ckPath + 'ckplayer/ckplayer.swf', divId, width, height, '10.0.0', ckPath + 'ckplayer/expressInstall.swf', flashvars, params, attributes);
}
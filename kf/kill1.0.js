kf['kill'] = function(config){
	...
	if (browser != '' && config.indexOf(browser) > -1) {
		this.init();
	}
};
kf.augment(kf['kill'], {
	init: function(){
		if ($('.kf-mask').length == 0) {
			this.mask = $('<div class="kf-mask fn-hide" style="position:absolute;left:0;right:0;top:0;bottom:0;z-index:90;background-color:#000;opacity:0.5;filter:alpha(opacity=50);"></div>');
			this.mask.css({
	...
	// 显示
	show: function(){
		this.kill.removeClass('fn-hide');
		this.mask.removeClass('fn-hide');
	},
	// 隐藏
	hide: function(){
		this.kill.addClass('fn-hide');
